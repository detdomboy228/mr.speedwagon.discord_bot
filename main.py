...
async def mem(ctx):
    try:
        if len(ctx.message.content.split()) == 2:
            ...
            n = int(ctx.message.content.split()[1]) - 1
            res = (requests.post('https://api.imgflip.com/caption_image', data={'template_id': (requests.get('https://api.imgflip.com/get_memes')).json()['data']['memes'][n]['id'], 'username': 'mr.speedwagon', 'password': 'D1scordB0t', 'text0': ctx.message.content.split('/m')[-1].split('/r')[0], 'text1': ctx.message.content.split('/m')[-1].split('/r')[1]})).json()
